  ...
  missedEl.innerHTML = missed.length
    ? missed.map(m => `
        <div class="missedItem">
          ...
          <div style="color:#a9b4d0;margin-top:4px;">Missed: ${escapeHtml(m.when)}</div>
          <div style="color:#a9b4d0;margin-top:2px;font-size:12px;">(red → purple on timeline)</div>
        </div>
      `).join("")
    ...
    <tbody>
      ${filtered.map(task => {
        const row = task.cells.map(cell => {
          ...
          return `<td><span class="cellBox" title="${escapeHtml(cell.date)} • ${escapeHtml(cell.bg)}" style="background:${bg}"></span></td>`;
        }).join("");
        return `<tr>
